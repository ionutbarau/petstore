...
class WebDriver {
    ...
    static attachToSession(executor, sessionId, opt_flow) {
        return acquireSession(executor,
            new command.Command(command.Name.DESCRIBE_SESSION).setParameter('sessionId', sessionId),
    ...
    static createSession(executor, desiredCapabilities, opt_flow) {
        return acquireSession(executor,
            new command.Command(command.Name.NEW_SESSION).setParameter('desiredCapabilities', desiredCapabilities),
   ...
   *     }
    ...
    findElementInternal_(locatorFn, context) {
        return this.call(() = > locatorFn(context)
    ).
...
class WebElement {
   ...
   *     }
    ...
    sendKeys(var_args) {
        ...
        let keys = promise.all(Array.prototype.slice.call(arguments, 0)).then(keys = > keys.map(String)
    )
        ;
