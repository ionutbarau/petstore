...
describe('promise', function () {
    ...
    const assertIsPromise = (p) =
    >
    assert.ok(promise.isPromise(p));
    const assertNotPromise = (v) =
    >
    assert.ok(!promise.isPromise(v));
    ...
    describe('thenFinally', function () {
        it('nonFailingCallbackDoesNotSuppressOriginalError', function () {
            ...
            }).thenCatch(done).thenFinally(done.assertCalled);
        });

        it('failingCallbackSuppressesOriginalError', function () {
            var done = callbackHelper(assertIsStubError);
            return promise.rejected(new Error('original')).thenFinally(throwStubError).thenCatch(done).thenFinally(done.assertCalled);
        });

        it('callbackThrowsAfterFulfilledPromise', function () {
            var done = callbackHelper(assertIsStubError);
            return promise.fulfilled().thenFinally(throwStubError).thenCatch(done).thenFinally(done.assertCalled);
        });
    ...
    describe('testLongStackTraces', function () {
        ...
        it('appendsEachStepToRejectionError', function () {
            ...
            }).then(fail).thenCatch(function (e) {
                throw e;
            }).then(fail).thenCatch(function (e) {
                throw e;
            }).then(fail, function (e) {
