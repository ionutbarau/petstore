...
test.suite(function (env) {
    ...
    test.ignore(env.browsers(Browser.IE, Browser.OPERA, Browser.SAFARI)).describe('manual proxy settings', function () {
        // phantomjs 1.9.1 in webdriver mode does not appear to respect proxy
        // settings.
        test.ignore(env.browsers(Browser.PHANTOM_JS)).it('can configure HTTP proxy host', function () {
            driver = env.builder().setProxy(proxy.manual({
                http: proxyServer.host()
            })).build();

            driver.get(helloServer.url());
            assert(driver.getTitle()).equalTo('Proxy page');
            assert(driver.findElement({tagName: 'h3'}).getText()).equalTo('This is the proxy landing page');
        });

        // PhantomJS does not support bypassing the proxy for individual hosts.
        test.ignore(env.browsers(Browser.PHANTOM_JS)).it('can bypass proxy for specific hosts', function () {
            driver = env.builder().setProxy(proxy.manual({
                http: proxyServer.host(),
            ...
            assert(driver.findElement({tagName: 'h3'}).getText()).equalTo('Hello, world!');
    ...
    test.ignore(env.browsers(
        Browser.IE, Browser.OPERA, Browser.PHANTOM_JS, Browser.SAFARI)).describe('pac proxy settings', function () {
        test.it('can configure proxy through PAC file', function () {
            driver = env.builder().setProxy(proxy.pac(proxyServer.url('/proxy.pac'))).build();
            ...
            assert(driver.findElement({tagName: 'h3'}).getText()).equalTo('Goodbye, world!');
        });
    });
